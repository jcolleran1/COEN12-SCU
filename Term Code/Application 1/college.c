//Joshua Colleran 
//Term project application 1: college.c
...
// It will then generate a random age to search for and then delete
// It will then show the largest age gap and free the data set
int main(void) {	
	LIST *list = createDataSet();  
    	assert(list != NULL);
    	srand(time(NULL));
    	int gap, i, randAge, randId, randNext, min, max;
	...
	randAge = (rand() % 13)+18;
	searchAge(list, randAge);
	deletion(list, randAge);
	gap = maxAgeGap(max, min);
	printf("Age gap: %d\n", gap);
	destroyDataSet(list);
}
